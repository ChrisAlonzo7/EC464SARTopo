...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (data == "ON") {
      strip_1.setPixelColor(0, strip_1.Color(191, 0, 255)); // Set the LED to red
    } else if (data == "OFF") {
      strip_1.setPixelColor(0, strip_1.Color(0, 0, 0)); // Turn off the LED
    }
    strip_1.show(); // Update the LED with the new color
  } // Update the first LED strip with the new colors
  ...
  delay(3500); // Wait for 1 second before sending the next message
